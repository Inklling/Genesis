...
class TestPostMergeHook:
    """Tests for post-merge COLLAB.md hook."""
    ...
    def test_install_post_merge_hook(self, git_repo):
        """install_post_merge_hook creates post-merge file."""
        from wiz.hooks import install_post_merge_hook, HOOK_MARKER
        msg = install_post_merge_hook(git_repo)
    ...
    def test_post_merge_idempotent(self, git_repo):
        """Installing post-merge twice updates existing hook."""
        from wiz.hooks import install_post_merge_hook
        install_post_merge_hook(git_repo)
        msg = install_post_merge_hook(git_repo)
        assert "Updated" in msg

    def test_uninstall_post_merge_hook(self, git_repo):
        """uninstall_post_merge_hook removes wiz post-merge hook."""
        from wiz.hooks import install_post_merge_hook, uninstall_post_merge_hook
        install_post_merge_hook(git_repo)
        msg = uninstall_post_merge_hook(git_repo)
        hook = git_repo / ".git" / "hooks" / "post-merge"
        assert not hook.exists()
        assert "Removed" in msg

    def test_uninstall_post_merge_no_hook(self, git_repo):
        """uninstall_post_merge_hook raises FileNotFoundError when absent."""
        from wiz.hooks import uninstall_post_merge_hook
        with pytest.raises(FileNotFoundError):
            uninstall_post_merge_hook(git_repo)

    def test_post_merge_foreign_blocked(self, git_repo):
        """install_post_merge_hook refuses foreign hook without force."""
        from wiz.hooks import install_post_merge_hook
        hook = git_repo / ".git" / "hooks" / "post-merge"
        hook.write_text("#!/bin/sh\necho foreign\n", encoding="utf-8")
        with pytest.raises(FileExistsError):
            install_post_merge_hook(git_repo, force=False)

    def test_post_merge_force_overwrites(self, git_repo):
        """install_post_merge_hook with force=True overwrites foreign hook."""
        from wiz.hooks import install_post_merge_hook, HOOK_MARKER
        hook = git_repo / ".git" / "hooks" / "post-merge"
        hook.write_text("#!/bin/sh\necho foreign\n", encoding="utf-8")
        install_post_merge_hook(git_repo, force=True)
        content = hook.read_text(encoding="utf-8")
        assert HOOK_MARKER in content

    def test_post_merge_is_executable(self, git_repo):
        """Post-merge hook has executable permission on Unix."""
        from wiz.hooks import install_post_merge_hook
        install_post_merge_hook(git_repo)
        hook = git_repo / ".git" / "hooks" / "post-merge"
        if sys.platform != "win32":
    ...
    def test_both_hooks_coexist(self, git_repo):
        """pre-commit and post-merge hooks can be installed independently."""
        from wiz.hooks import install_hook, install_post_merge_hook
        install_hook(git_repo)
        install_post_merge_hook(git_repo)
        pre = git_repo / ".git" / "hooks" / "pre-commit"
        post = git_repo / ".git" / "hooks" / "post-merge"
        assert pre.exists()
        assert post.exists()
        assert "wiz scan" in pre.read_text(encoding="utf-8")
        assert "COLLAB" in post.read_text(encoding="utf-8")
