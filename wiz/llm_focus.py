...
def run_micro_queries(
    queries: list[MicroQuery],
    filepath: str,
    cost_tracker=None,
    max_cost_usd: float = 0.01,
) -> list[Finding]:
    """Run micro-queries against LLM and return additional findings.

    Each query sends a small snippet + specific question.
    Caps total cost at max_cost_usd.

    Returns [] if no API key or queries are empty.
    """
    if not queries:
        return []
    ...
    findings = []
    total_tokens = 0
    token_budget = int(max_cost_usd / (15.0 / 1_000_000))  # rough output token budget

    for query in queries:
        if total_tokens + query.estimated_tokens > token_budget:
            break

        try:
            from .llm import micro_query_llm
            result, tracker = micro_query_llm(
                query.snippet, query.question, filepath,
                cost_tracker=cost_tracker,
            )
            if result:
                for f in result.get("findings", []):
                    from .config import Severity, Category, Source, Confidence
                    try:
                        findings.append(Finding(
                            file=filepath,
                            line=f.get("line", query.line_start),
                            severity=Severity(f.get("severity", "warning")),
                            category=Category(f.get("category", "bug")),
                            source=Source.LLM,
                            rule=f.get("rule", "llm-micro"),
                        ...
                        continue
            total_tokens += query.estimated_tokens
        except Exception:
            continue
