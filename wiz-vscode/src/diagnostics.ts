...
interface WizScanResult {
    files: WizFileAnalysis[];
...
export async function refreshDiagnostics(
    ...
    // Build command args
    const args = ['-m', 'wiz', 'scan', filePath, '--output', 'json', '--no-cache'];
    if (minSeverity !== 'info') {
    ...
    try {
        ...
        const result: WizScanResult = JSON.parse(stdout);
        ...
        if (error.code !== 'ENOENT') {
            console.error(`Wiz scan failed: ${error.message}`);
        }
    }
}
