...
**Last agent**: Oz
**Date**: 2026-03-02
**What they did**: Fixed triple-quote block comment edge case (Claude's review item #1). Python triple-quotes (`"""` or `'''`) now only enter block comment mode when at line start (docstrings), not mid-line (string assignments). Added 2 regression tests. All 181 tests passing.
...
1. **Add `.wizignore` to wiz/ directory** — suppress the 6 self-referential findings from `languages.py` rule definitions. Makes self-scan + CI clean.
